...
<script src="urljump.js"></script> <!-- 外部JavaScript -->
...
<?php
//どこにフォーカスをあてるかを決める
if ($mode == "edit") print "<body onLoad=\"document.f_edit.content.focus();FixedMidashi.create();\">";
elseif ($mode == "view" and $edit_yoko != "" and $edit_tate != "") print "<body onLoad=\"document.getElementById('{$edit_yoko}_{$edit_tate}').focus();FixedMidashi.create();\">";
elseif ($mode == "title_edit") print "<body onLoad=\"document.f_title_edit.content.focus();FixedMidashi.create();\">";
//elseif ($mode == "plane") print "<body onLoad=\"changeURL();FixedMidashi.create();\">";
else print "<body onLoad=\"FixedMidashi.create();\">";
...
if ($page_mae > 0) {
?>
[<a href="griddata_list.php?id=<?= $id ?>&category=<?= $category ?>&page=<?= $page_mae ?>&mode=<?= $mode ?>">(<?= $page_mae ?>)≪前頁</a>｜
<?php
}
else {
?>
...
｜<a href="griddata_list.php?id=<?= $id ?>&category=<?= $category ?>&page=<?= $page_saki ?>&mode=<?= $mode ?>">次頁≫(<?= $page_saki ?>)</a>]
　◆頁ビュー◆
<a href='javascript:table_trance()'>行と列を入替</a>（入替た状態では編集不可）
</form>
<b><a href="griddata_list.php?id=<?=$id?>&category=<?=$category-1?>&page=<?=$page?>&mode=view">【</a><?=$title?>
<a href="griddata_list.php?id=<?=$id?>&category=<?=$category+1?>&page=<?=$page?>&mode=view">】</a>《
<?php 
if ($data[0][0][$page] != "") {
...
<?php
	...
	print "<a id=0_0 href=\"griddata_list.php?id={$id}&category={$category}&page={$page}&mode=title_edit\">{$sub_title}</a>";
}
?>
...
?>
<TABLE border="1" _fixedhead="rows:1; cols:2">
<tr bgcolor="#BFC5CA">
<?php
	$start_cell = ($page != 0) ? 1 : 0;  
	if ($mode == "plane") {
		print "<TD><a href=\"griddata_list.php?id={$id}&category={$category}&page={$page}&mode=view\"><div align=\"center\">" . "編" . "</div></a></td>\n";
	}
	else {
		print "<TD><a href=\"griddata_list.php?id={$id}&category={$category}&page={$page}&mode=plane\"><div align=\"center\">" . "閲" . "</div></a></td>\n";
	}
	...
	($midashi == "short" and $tate_title[$i] <> "") ? $tate_midashi =  $tate_title[$i] : $tate_midashi =  $i . "行 " . $tate_title[$i];
...
?>
<td style="width: 200px; height: 200px; border: solid thin pink; margin: 0px; padding: 0px;">
<?php
			}
print "<div class=\"{$timediff_color[$j][$i][$page]}\">"
?>
...
<input type="hidden" name="conf_mode" value="<?= $conf_mode ?>">
<textarea name="content" cols="<?=$t_cols?>" rows="<?=$t_rows?>" ondblclick="ClickUrl()" id="content"  style=" margin: 0px; padding: 0px; width: 100%; height: 200px; box-sizing: border-box;">
...
<?php if ($time[$j][$i][$page]) echo substr($time[$j][$i][$page],0,16), "更新"; ?>
</form></div></td>
<?php
		}
		elseif ($mode == "plane") {
...
//			print "<td bgcolor=\"{$timediff_color[$j][$i][$page]}\"><div class=\"{$place}\">" . get_form($cell_data_out) . "</div></td>\n";
		}
		elseif ($timediff_color[$j][$i][$page] != "") {
			print "<td bgcolor=\"{$timediff_color[$j][$i][$page]}\"><a href=\"griddata_list.php?id={$id}&category={$category}&page={$page}&mode=edit&edit_yoko={$j}&edit_tate={$i}{$jump_name}\" id=\"{$j}_{$i}\"><div class=\"{$place}\">" . get_form($cell_data_out) . "</div></a></td>\n";
		}
		elseif ($master != 1) {
			print "<td><a href=\"griddata_list.php?id={$id}&category={$category}&page={$page}&mode=edit&edit_yoko={$j}&edit_tate={$i}{$jump_name}\" id=\"{$j}_{$i}\"><div class=\"{$place}\">" . get_form($cell_data_out) . "</div></a></td>\n";
		}
		else {
			print "<td bgcolor=\"skyblue\"><a href=\"griddata_list.php?id={$id}&category={$category}&page={$page}&mode=edit&edit_yoko={$j}&edit_tate={$i}{$jump_name}\" id=\"{$j}_{$i}\"><div class=\"{$place}\">" . get_form($cell_data_out) . "</div></a></td>\n";
		}
	}
	print "</tr>\n";
	$cell_data_csv .= "\n";
}
	echo "<tr bgcolor=\"#BFC5CA\">";
	if ($mode == "plane") {
		print "<TD><a href=\"griddata_list.php?id={$id}&category={$category}&page={$page}&mode=view\"><div align=\"center\">" . "編" . "</div></a></td>\n";
	}
	else {
...
&copy; 塘　誠(Makoto Tomo),2006-2007.
<?php
if ($mode == "plane") {
?>
...
?>
<a href="griddata_imp.php?id=<?=$id?>&category=<?=$category?>&page=<?=$page?>">表形式インポート</a>　
<?php
	print "<a href=\"griddata_list.php?id={$id}&category={$category}&page={$page}&pmode=xls\">Excelで開く</a>\n";
?>
</body>
</html>
