...
<title>列：<?= $yoko_in ?> <?= $data[$yoko_in][0][0] ?>-<?= $title ?></title>
<style type="text/css">
<!--
...
//ページ移動リンクの作成
if ($yoko_in == 0) print "<p><b>ページマスター（列共通）</b></p>\n";
?>
...
if ($yoko_mae > 0) {
?>
[<a href="griddata_yoko.php?id=<?= $id ?>&category=<?= $category ?>&yoko=<?= $yoko_mae ?>&mode=<?= $mode ?>">(<?= $yoko_mae ?>)≪前</a>｜
<?php
}
else {
?>
...
?>
		<option value="<?=$p?>"<?php if ($p == $yoko_in) print " selected" ?>><?=$p?>列 <?=$data[$p][0][0]?></option>
<?php
	}
?>
	</select>
...
｜<a href="griddata_yoko.php?id=<?= $id ?>&category=<?= $category ?>&yoko=<?= $yoko_saki ?>&mode=<?= $mode ?>">次≫(<?= $yoko_saki ?>)</a>]
　〓列方向ビュー〓
...
<?php
	...
	if ($mode == "plane") {
		print "<TD><a href=\"griddata_yoko.php?id={$id}&category={$category}&yoko={$yoko_in}&mode=view\"><div align=\"center\">" . "編" . "</div></a></td>\n";
	}
	else {
		print "<TD><a href=\"griddata_yoko.php?id={$id}&category={$category}&yoko={$yoko_in}&mode=plane\"><div align=\"center\">" . "閲" . "</div></a></td>\n";
	}

	$cell_data_csv .= "\"" . $sub_title . "\"\t";
	for( $j=$start_cell; $j<=$max_page+1; $j++) {//横
//		print "<TD><a href=\"griddata_list.php?id={$id}&category={$category}&page={$j}\"><div align=\"center\">{$j}頁<br>{$page_title[$j]}</div></a></td>\n";
//		$cell_data_csv .= "\"" . $page_title[$j] . "\"\t";
		if ($midashi == "short" and $page_title[$j] <> "") {
			$yoko_midashi =  $page_title[$j];
		}
		else if ($page_title[$j] <> "") {
			$yoko_midashi =  $j . "頁<br>" . $page_title[$j];
		}
		else {
			$yoko_midashi =  $j . "頁";
		}
...
//	print "<TD bgcolor=\"#BFC5CA\"><a href=\"griddata_tate.php?id={$id}&category={$category}&tate={$i}\"><div align=\"left\">{$i}行 {$tate_title[$i]}</div></a></td>\n";
//	$cell_data_csv .= "\"" . $tate_title[$i] . "\"\t";
	...
	for( $j=$start_cell; $j<=$max_page+1; $j++) {//横
		...
		elseif ($yoko_in != 0 and ($data[$yoko_in][$i][0] != "" or $data[0][$i][$j] != "" or $data[$yoko_in][0][$j] != "")) { //頁マスター
			$master = 1;
...
<?php if ($time[$yoko_in][$i][$j]) echo substr($time[$yoko_in][$i][$j],0,16) ,"更新"; ?>
</form></div></td>
<?php
		}
		elseif ($mode == "plane") {
...
?>
<a href="griddata_imp.php?id=<?=$id?>&category=<?=$category?>&yoko=<?=$yoko?>">表形式インポート</a>　
<?php
	print "<a href=\"griddata_yoko.php?id={$id}&category={$category}&yoko={$yoko_in}&pmode=xls\">Excelで開く</a>\n";
?>
</body>
</html>
