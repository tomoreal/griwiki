...
<?php //		if ($num_rows != 0) { ?>
<form name="f_page_select" method="post" action="griddata_list2.php?id=<?= $id ?>&category=<?= $category ?>">
<?php
if ($page_mae > 0) {
?>
[<a href="griddata_list2.php?id=<?= $id ?>&category=<?= $category ?>&page=<?= $page_mae ?>">(<?= $page_mae ?>)≪前頁</a>｜
<?php
}
else {
?>
...
｜<a href="griddata_list2.php?id=<?= $id ?>&category=<?= $category ?>&page=<?= $page_saki ?>">次頁≫(<?= $page_saki ?>)</a>]
　◆頁ビュー◆
<a href="griddata_yoko2.php?id=<?= $id ?>&category=<?= $category ?>">列</a>
...
<input name="content" type="text" onBlur="document.f_title_edit.submit()" size="30" value="<?= $data[0][0][$page] ?>" id="0_0">
...
<span id='ichi[0][0]' style='color:#000;' ondblclick="updateform('ichi[0][0]', '<?=get_form($data[0][0][$page])?>','<?= $id ?>','<?= $category ?>','<?= $page ?>','0','0','<?= $time[0][0][$page] ?>','<?= $conf_mode ?>')"><?=get_form($data[0][0][$page])?></span>
<noscript>
<?php
...
?>
</b>(現在：<?= date("Y-m-d H:i") ?>｜最終更新：<?= substr($max_time,0,16) ?>)文字列のダブルクリックで編集</p>
<TABLE border="1">
<tr bgcolor="#BFC5CA">
<?php
	$start_cell = ($page != 0) ? 1 : 0;  
	print "<TD><a href=\"griddata_list2.php?id={$id}&category={$category}&page={$page}&mode=view\"><div align=\"center\">" . "■" . "</div></a></td>\n";
...
<span id='ichi[<?=$j?>][<?=$i?>]' style='color:#000;' ><a href="javascript:updateform('ichi[<?=$j?>][<?=$i?>]', '<?=get_cr($cell_data)?>','<?= $id ?>','<?= $category ?>','<?= $page ?>','<?= $i ?>','<?= $j ?>','<?= $time[$j][$i][$page] ?>','<?= $conf_mode ?>')"><?=get_form($cell_data_out)?></a></span>
<noscript>
<?php
