...
//マス目のデータを配列に読み込む
$rs = pg_query($con, "select *, date_trunc('second',time) as time_s from $griddata where owner = '$id' and category='$category' and tate = $tate");
...
function updateform(upid, oldstr,owner,category,page,tate,yoko,time,conf_mode,title_edit_mode)
{
	...
	nakami += "<input type='hidden' name='edit_mode' value='tate'>";
...
<title>行：<?= $tate ?> <?= $data[0][$tate][0] ?>-<?= $title ?></title>
<style type="text/css">
<!--
...
//ページ移動リンクの作成
if ($tate == 0) print "<p><b>ページマスター（行共通）</b></p>\n";
?>

<form name="f_tate_select" method="post" action="griddata_tate2.php?id=<?= $id ?>&category=<?= $category ?>">
<?php
if ($tate_mae > 0) {
?>
[<a href="griddata_tate2.php?id=<?= $id ?>&category=<?= $category ?>&tate=<?= $tate_mae ?>">(<?= $tate_mae ?>)≪前</a>｜
<?php
}
else {
?>
...
?>
		<option value="<?=$p?>"<?php if ($p == $tate) print " selected" ?>><?=$p?>行 <?=$data[0][$p][0]?></option>
<?php
	}
?>
	</select>
...
｜<a href="griddata_tate2.php?id=<?= $id ?>&category=<?= $category ?>&tate=<?= $tate_saki ?>">次≫(<?= $tate_saki ?>)</a>]
　‖縦方向ビュー‖
<a href="griddata_list2.php?id=<?= $id ?>&category=<?= $category ?>">頁</a>
<a href="griddata_yoko2.php?id=<?= $id ?>&category=<?= $category ?>">列</a> 欄のクリックでビューを切替可
</form>
<p><b>【<?= $title ?>】《
<?php 
if ($data[0][$tate][0] != "") {
	$sub_title = $data[0][$tate][0];
	$conf_mode = "update";
}
else {
	$sub_title = "＿＿";
...
<input type="hidden" name="rep_tate" value="<?= $tate ?>">
<input type="hidden" name="rep_yoko" value="0">
<input type="hidden" name="time" value="<?= $time[0][$tate][0] ?>">
...
<input name="content" type="text" onBlur="document.f_title_edit.submit()" size="30" value="<?= $data[0][$tate][0] ?>">
...
<?php
	...
	print "<a href=\"griddata_tate.php?id={$id}&category={$category}&edit_tate={$tate}&mode=title_edit\">{$sub_title}</a>";
}
?>
...
<tr bgcolor="#BFC5CA">
<?php
	$start_cell = ($tate != 0) ? 1 : 0;  
	print "<TD><a href=\"griddata_tate2.php?id={$id}&category={$category}&tate={$tate}&mode=view\"><div align=\"center\">" . "■" . "</div></a></td>\n";
	for( $j=$start_cell; $j<=$max_yoko+1; $j++) {//横
		print "<TD><a href=\"griddata_yoko2.php?id={$id}&category={$category}&edit_yoko={$j}\"><div align=\"center\">{$j}列<br>{$yoko_title[$j]}</div></a></td>\n";
	}
	print "</tr>\n";
for( $i=$start_cell; $i<=$max_page+1; $i++) {//縦
	print "<TD bgcolor=\"#BFC5CA\"><a href=\"griddata_list2.php?id={$id}&category={$category}&page={$i}\"><div align=\"left\">{$i}頁 {$page_title[$i]}</div></a></td>\n";
	for( $j=$start_cell; $j<=$max_yoko+1; $j++) {//横
		$master = 0;
		if ($data[$j][$tate][$i] == "") {
			$conf_mode = "insert";
		...
		if ($data[$j][$tate][$i] != "") {
			$cell_data =  $data[$j][$tate][$i];
			$cell_data_out =  $data[$j][$tate][$i];
		}
		elseif ($tate != 0 and ($data[$j][$tate][0] != "" or $data[0][$tate][$i] != "" or $data[$j][0][$i] != "")) { //頁マスター
			$master = 1;
			$cell_data = $data[$j][$tate][0] . $data[0][$tate][$i]. $data[$j][0][$i];
			$cell_data_out = $data[$j][$tate][0] . $data[0][$tate][$i]. $data[$j][0][$i];
		}
		else {
			$cell_data =  $data[$j][$tate][$i];
			$cell_data_out =  "^＿";
...
<?php
			...
			elseif ($timediff_color[$j][$tate][$i] == 1) {
?>
<td bgcolor="yellow">
<?php
			}
...
<input type="hidden" name="time" value="<?= $time[$j][$tate][$i] ?>">
<input type="hidden" name="conf_mode" value="<?= $conf_mode ?>">
<input type="hidden" name="edit_mode" value="tate">
...
<?php
		...
		elseif ($timediff_color[$j][$tate][$i] != "") {
?>
<td>
<div class="todolist" id="idid"><div class="<?=$timediff_color[$j][$tate][$i]?>"><div class="<?=$place?>">
<span id='ichi[<?=$j?>][<?=$i?>]' style='color:#000;' ><a href="javascript:updateform('ichi[<?=$j?>][<?=$i?>]', '<?=get_cr($cell_data)?>','<?= $id ?>','<?= $category ?>','<?= $i ?>','<?= $tate ?>','<?= $j ?>','<?= $time[$j][$tate][$i] ?>','<?= $conf_mode ?>')"><?=get_form($cell_data_out)?></a></span>
<noscript>
<?php
	print "<br><a href=\"griddata_tate.php?id={$id}&category={$category}&tate={$tate}&mode=edit&edit_yoko={$j}&edit_tate={$i}\" id=\"{$j}_{$i}\"><div class=\"{$place}\">" . get_form($cell_data_out) . "</div></a>\n";
?>
</noscript>
</div></div></div>
</td>
<?php
//			print "<td><div class=\"{$timediff_color[$j][$tate][$i]}\"><a href=\"griddata_tate.php?id={$id}&category={$category}&tate={$tate}&mode=edit&edit_yoko={$j}&edit_tate={$i}\" id=\"{$j}_{$i}\"><div class=\"{$place}\">" . get_form($cell_data_out) . "</div></a></div></td>\n";
		}
		elseif ($master != 1) {
?>
<td>
...
<?php
//			print "<td><a href=\"griddata_tate.php?id={$id}&category={$category}&tate={$tate}&mode=edit&edit_yoko={$j}&edit_tate={$i}\" id=\"{$j}_{$i}\"><div class=\"{$place}\">" . get_form($cell_data_out) . "</div></a></td>\n";
		}
		else {
?>
<td bgcolor="skyblue">
...
<?php
//			print "<td bgcolor=\"skyblue\"><a href=\"griddata_tate.php?id={$id}&category={$category}&tate={$tate}&mode=edit&edit_yoko={$j}&edit_tate={$i}\" id=\"{$j}_{$i}\"><div class=\"{$place}\">" . get_form($cell_data_out) . "</div></a></td>\n";
		}
	}
	print "</tr>\n";
}
